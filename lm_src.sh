...
Usage()
{
    echo "Usage: $0 [-d <no|pf|mdev> ] [-i <no|lega|scal>] [-u <mdev-uuid>]" 1>&2;
    exit 1;
}
...
if [ $DEV == "mdev" ] && [ $UUID == "off" ]; then
	echo "device UUID is missing"
	Usage
fi
...
if [ -n "${IOMMU}" ]; then
	...
	lega)
	    ...
	    ;;
	scal)
	    QEMU_CMDLINE_IOMMU="-device vfio-pci,sysfsdev=$MDEV_PATH/$UUID,x-enable-migration=on,x-enable-dynamic-mmap=on"
	    ;;
    esac
fi
