...
if [ -n "${IOMMU}" ]; then
	...
	lega)
	    ...
	    ;;
	scal)
	    QEMU_CMDLINE_IOMMU="-device intel-iommu,caching-mode=on,dma-drain=on,x-scalable-mode="modern",device-iotlb=on,aw-bits=48"
	    ;;
    esac
fi
...
$QEMU_IMAGE \
	...
	-trace "vtd_reg_write_*" \
	-D /home/aubrey/kvm/qemu.log \
	-kernel ${KERNEL_IMAGE} \
